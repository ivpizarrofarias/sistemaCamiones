...
@Component({
  ...
  `,
  styles: [`
    .page-container { padding: 20px; max-width: 600px; margin: auto; }
...
export class ClientUpdateComponent implements OnInit {
  ...
  private loadClient(id: number): void {
    this.clientService.getClientById(id).subscribe({
      next: (response: any) => {
        // Si tu backend devuelve { data: Client }
        const client: Client = response.data ? response.data : response;
      ...
      error: err => {
        ...
        this.snackBar.open('No se pudo cargar el cliente', 'Cerrar', { duration: 3000 });
      }
    });
  }
  ...
  update(): void {
    ...
    const payload: Client = {
      clientName: this.capitalizeWords(this.form.value.clientName!),
